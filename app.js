class ReportFormApp {
    ...
    initEventListeners() {
        ...
        document.getElementById('back-to-main-full-report')?.addEventListener('click', () => {
            // Clear full report authentication fields when going back to main screen
    ...
    validateForm() {
        ...
        requiredFields.forEach(field => {
            ...
            if (!hasValue) {
                ...
                console.log('Field missing value:', fieldName);
    ...
    async exportPDFWithFileSystemAPI(element, opt, filename) {
        try {
            // Generate PDF as blob
            const pdfBlob = await html2pdf().set(opt).from(/** @type {HTMLElement} */(element)).outputPdf('blob');
            ...
            await html2pdf().set(opt).from(/** @type {HTMLElement} */(element)).save();
        }
    }
    ...
    async generateFullReport(reports, container) {
        try {
            ...
            reports.sort((a, b) => {
                try {
                    const dateA = new Date(b.metadata.last_modified || b.metadata.date_created);
                    const dateB = new Date(a.metadata.last_modified || a.metadata.date_created);
                    return dateA.getTime() - dateB.getTime();
                } catch (error) {
                    return 0;
                }
    ...
    authenticateFullReport() {
        const usernameElement = document.getElementById('full-report-username');
        const passwordElement = document.getElementById('full-report-password');
        ...
        const username = usernameElement instanceof HTMLInputElement ? usernameElement.value : '';
        const password = passwordElement instanceof HTMLInputElement ? passwordElement.value : '';
        ...
        if (!username || !password) {
            ...
            if (!password) {
                this.showFieldError(passwordElement, 'Заполните все поля');
            }
            return;
        }
    ...
    authenticateRetrospective() {
        const usernameElement = document.getElementById('retrospective-username');
        const passwordElement = document.getElementById('retrospective-password');
